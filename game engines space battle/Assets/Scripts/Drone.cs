...
public class Drone : MonoBehaviour
{
    ...
    [SerializeField] private Vector3[] checksurroundings;

    private List<Drone> cohesionNeighbours = new List<Drone>();
    private List<Drone> avoidanceNeighbours = new List<Drone>();
    private List<Drone> aligementNeighbours = new List<Drone>();
    ...
    public void MoveUnit()
    {
        ...
        moveVector = Vector3.SmoothDamp(myTransform.forward, moveVector, ref currentVelocity, damping);
    ...
    private Vector3 CalculateObstacleVector()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(myTransform.position, myTransform.forward, out hit, assignedFlock.obstacleDistance, obstacles))
        {
    ...
    private Vector3 FindBestDirectionToAvoidObstacle()
    {
        ...
        for (int i = 0; i < checksurroundings.Length; i++)
        {

            RaycastHit hit;
            var currentDirection = myTransform.TransformDirection(checksurroundings[i].normalized);
            if (Physics.Raycast(myTransform.position, currentDirection, out hit, assignedFlock.obstacleDistance, obstacles))
            {
    ...
    {
        return Vector3.Angle(myTransform.forward, position - myTransform.position) <= fieldofview;
    }
}
